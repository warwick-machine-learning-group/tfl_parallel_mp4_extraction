'''
* Filename: run_4_parallel_video_extraction.py
* Author: Edward Chuah
...
class VidLdr(object):
  ...
  def run_on_4_processors(self, in_dir, out_dir, number_videos, all_videos_frame_count, number_frames_elements, all_videos_frame_data):
...
def main(argv):
  ...
  for opt, arg in opt_list:
    if opt == '-h':
      print('mpirun -np 4 python run_4_parallel_video_extraction.py -i <input_directory> -o <output_directory>')
      sys.exit()
    if opt in ("-i", "--idir"):
      input_dir = arg
    if opt in ("-o", "--odir"):
  ...
  v.run_on_4_processors(input_dir, output_dir, number_videos, all_videos_frame_count, number_frames_elements, all_videos_frame_data)
